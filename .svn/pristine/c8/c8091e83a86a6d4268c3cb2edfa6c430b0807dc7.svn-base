		       ...
		       var PaygridPanel = $('#dg').datagrid("getPanel");
	           	 ...
	           	 var row = $('#dg').datagrid('getSelected');
	   			   var id=row.id;
					 	$.messager.confirm("提示", "您确定要删除此数据?", function(r) {
							// 点击了确定按钮，r返回的结果为true
							if (r) {
								$.get("../pay/deleteReceivable/" + id, function(data) {
									if (data.success) {
										// 刷新表格
										$("#dg").datagrid("reload");
									} else {
				...
				function linkForm(company) {
					...
					var dialog = $("<div/>").dialog(
							{
								href : '../pay/payabledetails?company='+company,
								title :'应付帐详情',
								height : '90%',
								width : '100%',
								modal : true,
				...
				function receivableForm(id) {
					...
					var dialog = $("<div/>").dialog(
							{
								href : '../pay/addPaiable' + (id ? ("/" + id) : ""),
								title :id? "新增应付帐信息" :"编辑应付帐信息",
								height : '520',
								width : '500',
								modal : true,
								onClose : function() {
								...
								buttons: [{
									...
									handler: function() {
										...
										if (linkForm.form('validate')) {
											...
											$.post("../pay/addReceivableModel", linkForm
